...
class CoF_Sim_Result_for_Fixed_H_Fixed_P:
    def __init__(self, H_a, sum_rate_i_H, A, alpha_opt):
        self.H_a = H_a
    ...
    for i_H in range(0, iter_H):
        ...
        for i_P_prod in range(0, division_P**L):
            ...
            for i_dim_P in range(0, L):
                ...
                # FIX ME!
                # delte +0.1 in the following two lines!!!!
                P_mat[L-i_dim_P-1, L-i_dim_P-1] = sqrt(P_temp+0.01)
                P_vec[L-i_dim_P-1] = deepcopy(P_temp+0.01)
...
plot_sum_rate = list_plot(zip(P_eq_dB, sum_rate), plotjoined=True, marker='o', \
                          ...
                          title = 'Comparison of fixed and variable methods')
